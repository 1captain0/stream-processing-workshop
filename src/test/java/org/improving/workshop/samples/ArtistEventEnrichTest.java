...
class ArtistEventEnrichTest {
    ...
    private TestOutputTopic<String, ArtistEventEnrich.EventTicketAggregation> outputTopic;

    @BeforeEach
    void setUp() {
        // Initialize the TopologyTestDriver
        var streamsBuilder = new StreamsBuilder();
        ArtistEventEnrich.configureTopology(streamsBuilder);
        ...
        outputTopic = driver.createOutputTopic(
                ArtistEventEnrich.OUTPUT_TOPIC,
                Serdes.String().deserializer(),
                ArtistEventEnrich.EVENT_TICKET_AGGREGATION_SERDE.deserializer()
        );
    }

    @Test
    @DisplayName("Verify Event Ticket Output")
    void verifyEventTicketOutput() {
        // Example input data
        ...
        for (TestRecord<String, ArtistEventEnrich.EventTicketAggregation> record : outputRecords){
            System.out.println(record.getValue().getTheEvent() + " " + record.getValue().getTurnout() + " " +record.getValue().getCount());
        }
